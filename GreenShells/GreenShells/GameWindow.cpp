...
GameWindow* GameWindow::m_instance = new GameWindow();
...
void GameWindow::ShowWindow()
{
	...
	while (!m_doQuit)
	{
		if (m_resChanged)
		{
			GameWindow* temp = m_nextWindow;
			m_nextWindow = m_instance;
			m_instance = temp;
			////std::swap(this, newWindow);
			delete m_nextWindow;
		}
		else
		{
...
{
	m_resChanged = true;
	m_nextWindow = new GameWindow(newRes);
